...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
  ...
  public WheelPicker(Context context, AttributeSet attrs) {
    ...
    mItemTextSize = Helper.dp2px(context,16);
    mVisibleItemCount = 5;
    mSelectedItemPosition = 0;
    hasSameWidth = false;
    mTextMaxWidthPosition = -1;
    mMaxWidthText = null;
    mSelectedItemTextColor = Color.parseColor("#000000");
    mItemTextColor = Color.parseColor("#000000");
    mItemSpace = 0;
    isCyclic = false;
    hasIndicator = true;
    mIndicatorColor = Color.parseColor("#000000");
    mIndicatorSize = Helper.dp2px(context,1);
    hasCurtain = false;
    mCurtainColor = 0x88FFFFFF;
    hasAtmospheric = false;
    isCurved = false;
    mItemAlign = ALIGN_CENTER;
    fontPath = null;
    itemClick = false;
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_UP:
        ...
        if (isClick && !isForceFinishScroll) {
          if (itemClick) {
            // 点击y坐标与中心点y坐标的差距
            float diff = mWheelCenterY - event.getY();
            // 计算差距有多少个item
            int diffItemCount = Math.round(diff/mItemHeight);
            // diff > 0 向上; diff < 0 向下;
            int toBeItemPosition = mCurrentItemPosition - diffItemCount;
            // 边界处理
            if (diff == 0 || (diff > 0 && toBeItemPosition < 0) || (diff < 0 && toBeItemPosition > mData.size()-1)) break;
            // 滚动到指定position
            setSelectedItemPosition(toBeItemPosition);
            // 处理回调
            if (null != mOnItemSelectedListener)
              mOnItemSelectedListener.onItemSelected(this, mData.get(toBeItemPosition), toBeItemPosition);
            if (null != mOnWheelChangeListener) {
              mOnWheelChangeListener.onWheelSelected(toBeItemPosition);
            }
          }
          break;
        };
  ...
  @Override
  public void setItemClick(boolean bool) {
    itemClick = bool;
  }
